...
Gemini quotas vary by model, project and account. Check the active RPM (requests/minute), TPM (input tokens/minute) and RPD (requests/day) in AI Studio before provider tests; RPD resets at midnight Pacific. Quota exhaustion is an error: there is no automatic provider/model fallback. Standard Gemini text generation is the only remote model function; embeddings/reranking stay local and Search/Maps grounding, Batch, Vertex AI and paid features are not configured.
