...
**Default branch:** `main`
**Status:** NB-02 COMPLETE — NB-03 IN_PROGRESS; cloud project, billing association, cost, and billing-budget API approval required before provisioning
...
| NB-03 | Cloud baseline and least-privilege infrastructure | NB-02 | Officially validated Google project/services/IAM/secrets/logging; reproducible IaC; independently deployable/observable Hindsight API + worker service(s); shared Cloud SQL connectivity; no permanent GitHub cloud key | IN_PROGRESS |
...
The canonical architecture selects Google Cloud, Cloud SQL PostgreSQL + pgvector, Cloud Storage for recovery artifacts, and independently deployable Hindsight API/worker services. At preflight time, repository inspection found only `infra/cloud/database/postgres-schema.sql`; no IaC framework or cloud deployment configuration was present and no Terraform/OpenTofu/Pulumi executable was installed. OpenTofu 1.12.6 was installed locally afterward and NB-03 IaC was added under `infra/cloud/tofu/`.

No Nexus-approved Google Cloud project ID is recorded in this Blueprint or repository. The local gcloud default is outside the approved Nexus target; it was not used. No cloud resources, APIs, IAM, billing or secrets were changed. No Lumenva writes or PR #72 actions were performed.

This preflight's missing-decision state was superseded by the owner's NB-03 decision on 2026-09-26. Approved choices are Google Cloud, `europe-west1`, OpenTofu, Cloud Run, Cloud SQL PostgreSQL, Cloud Storage, Secret Manager, Artifact Registry, `nexus-<env>-<service>` naming, `dev`, least privilege, separate service accounts, no HA/GPU, conservative scaling, budget alerts, and DEV before PROD. The permitted API list is explicit. `billingbudgets.googleapis.com` is additionally required for a project-scoped budget through IaC and needs approval as an API allowlist exception.

NB-03 local IaC is under `infra/cloud/tofu/`. It defines separate Hindsight API/worker identities and runtime configuration (disabled until Secret Manager values exist), regional non-HA PostgreSQL, private versioned recovery/state buckets, per-secret access without payloads, a GHCR remote cache pinned to a Hindsight image digest, a Cloud Monitoring error alert, and a proposed USD 75 monthly billing alert. `infra/cloud/database/hindsight-bootstrap.sql` enables pgvector without credentials. Hindsight's full image requires 2 GiB for API and worker; one manual worker stays on continuously. Planning estimate is USD 35–60/month before Hindsight's external LLM usage, based on official prices and assumptions in the IaC README.

Provisioning is gated on owner approval of the existing Nexus-labeled project candidate, its billing association, the monthly cost estimate, and the billing-budget API exception. The candidate currently has billing disabled; no billing link, API enablement, IAM change, secret write, or cloud resource creation has occurred. Hindsight LLM provider and credential remain deployment inputs. NB-03 remains `IN_PROGRESS` until cloud integration, security, evidence, and Definition of Done gates pass.
