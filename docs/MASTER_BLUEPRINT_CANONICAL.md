...
The current MCG tracker reports F0–F2 and F4 accepted; F3, F5 and F6 remain partial: **4/7 accepted = 57% for the MCG workstream only**. This is not the overall Maestri/Brain/Governance percentage. All 14 registered dashboard areas have readable reports with source, scope, measurement quality and explicit unavailable/partial states; isolated browser checks cover report navigation, deep links, keyboard skip-link, theme selection and 320px mobile layout. F5 still needs screen-reader, physical touch-device and final reference-fidelity acceptance. F3 provider live-session verification and broader hostile-input coverage remain open. F6 security finding triage and release/rollback gates remain open. Jules drafts not merged/validated are not evidence.
...
- **F3 Token Firewall — partial:** existing operation traces, bounded `compactResult`, `waitForTerminal`, and compiler fragment hashes/deltas form the foundation. PRs #19, #22, #23, #25, #26, #28 and #29 provide result redaction/evidence, operation provenance, bounded read batches, path/size tests, SHA-256 `read-if-changed`, and one stdio MCP tool. The synchronized `main` now additionally redacts every summary field before serialization/truncation, applies field caps while preserving numeric token metrics, and provides a read-only `git-diff-if-changed` checkpoint over tracked staged/unstaged changes with SHA-256, disabled external diff/textconv, and truncation rejection. Project `.codex/config.toml` registers only `mcg_read_batch` with per-tool approval; wrapper and official MCP client tests verify discovery and bounded read. No live Codex model/tool turn or measured round-trip/token reduction is claimed. Remaining: live provider/tool-loop verification, broader hostile-input/fuzz coverage and adoption of checkpoint hashes by the router.
...
- **F6 Evaluation, benchmark and release gate — partial:** security workflows are merged and their recorded post-merge runs passed at `249cb391cd51c235f2489ffbb6f858c82774c336`; scanners are report-only and ZAP passive-only. The recorded 30-case validation-v2 paired run reports exact measurements and non-regression for that corpus only: baseline 1,028,032 vs. MCG 862,578 total tokens (16.09% measured difference), with 12.74% context-token difference. Treat these figures as corpus-specific; they do not prove universal savings or release approval. Security triage reports are documented separately in `docs/security/`; live alert findings remain to be reviewed. Remaining: scanner finding triage, broader evaluation, screen-reader/touch/reference acceptance under F5, and verified release/rollback gates.

### 49.2.1 Current implementation evidence (2026-09-25)

- F3 focused work is integrated in GitHub `main` at `2398bc1eaf27caf2d9c7f98a8766e85c98777b72`: `compactResult` redacts every summary field before truncation and preserves numeric token-economy metrics; Local Runtime provides hash-based, read-only checkpoints for tracked staged/unstaged Git diffs. Project Codex config allows only `mcg_read_batch` with per-tool approval; Codex CLI recognizes it, and official MCP-client tests cover discovery and bounded reads. Verification: MCG 53/53 tests; Local Runtime 30/30 and strict typecheck; syntax/import 35 modules; sensitive scan 203 files; Jazzer 62,897 runs in 9 seconds without a crash. GitHub MCG gates, CodeQL, security scanning, and fuzzing all passed on this SHA. F3 remains partial: no live Codex model/tool turn, no proof of model-round-trip reduction, broader hostile-input fuzzing and router checkpoint adoption remain open.
...
All seven phases require evidence. F0–F2 and F4 are accepted (4/7, 57%); F3, F5 and F6 remain partial. The overall Maestri program remains unmeasured until its workstreams have reconciled objective denominators.
