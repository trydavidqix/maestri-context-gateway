...
- F3 focused work is integrated into the local `main` history: `compactResult` redacts every summary field before truncation and preserves numeric token-economy metrics; Local Runtime provides hash-based, read-only checkpoints for tracked staged/unstaged Git diffs. Project Codex config allows only the `mcg_read_batch` MCP tool with per-tool approval; CLI discovery and MCP-client bounded-read tests pass. Verification: MCG 53/53 tests; Local Runtime 30/30 and strict typecheck; syntax/import 35 modules; sensitive scan 203 files; Jazzer 62,897 runs in 9 seconds without a crash. The commit has not yet been pushed, so remote CI remains pending. F3 stays partial: no live Codex model/tool turn, no proof of model-round-trip reduction, broader hostile-input fuzzing and router checkpoint adoption remain open.
