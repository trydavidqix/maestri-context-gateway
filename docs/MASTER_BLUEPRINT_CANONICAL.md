...
- **F3 Token Firewall — partial:** operation envelope; compact summaries connected to the executor path with evidence drill-through; safe batching of independent read-only operations; asynchronous wait without model polling; hash/checkpoint/delta reuse with full-read escape hatch; test redaction, large/hostile outputs, truncation, rename/removal, scope; benchmark candidate output cap only when installed CLI/schema supports it. Never change global Codex config without separate authorization.
- **F4 Clear reports for all 13 dashboard areas — partial, not accepted:** shared report shell and readable recursive fields/lists now cover Overview, History, Traces, Tasks, Agents, Tools, Plugins/Skills, MCPs, Context/Graph, Cache, Memory, Validation MCG and Alerts. Reports show source, timestamp, measurement quality, explicit unavailable fields, safe escaped content and empty states. Isolated browser smoke rendered all 13 areas without raw JSON. Remaining gates: per-metric period/coverage/limitation labels where applicable, keyboard/screen-reader drilldown, stale/error-state checks and complete visual QA.
- **F5 Reference-faithful design, themes and accessibility — implemented, not accepted:** exact light/dark semantic palettes and persistent Light/Dark/System selection are implemented; isolated Playwright confirmed palette/meta-color changes and 390px layout without horizontal overflow. Remaining: compare screenshots against supplied references, verify keyboard/screen-reader/contrast and reduced-motion behavior, touch behavior, and ensure statuses never depend on color alone. Do not control the user's active Chrome; use isolated browser/test tooling.
...
All seven phases require evidence. Dashboard has useful PT-BR reports across all 13 areas; numbers are qualified and link to evidence; light/dark references and usability are validated; token firewall reduces measured redundancy without harming quality; tests and scans pass. Overall plan remains unmeasured until all workstreams have a reconciled objective denominator.
...
- The prior standalone MCG master plan is preserved as a historical snapshot in `docs/archive/`; its seven phases, scope rules, status, exact palette, 13 report areas, token-firewall gates and validation/release requirements are integrated in §49.
...
| Current MCG master plan | §49, §50–51 | Scope, verified phase status, F0–F6, 13 dashboard reports, exact dual palettes/theme/accessibility, token firewall, benchmarks/release. Full prior file archived. |
