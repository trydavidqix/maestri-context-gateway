...
Current MCG status after this audit: F0–F2 and F4 accepted; F3, F5 and F6 partial: **4/7 accepted = 57% for MCG only**. This is not the overall Maestri/Brain/Governance percentage. All 14 dashboard areas have complete reports, explicit scope/limitations, source and measurement quality, unavailable states, and reloadable deep links. F4 is accepted. F5 still needs screen-reader and physical touch-device checks; screenshot comparison confirms the supplied 1280×720 layout and both palettes, not pixel-perfect equality. F3 remains unmerged and provider integration, Git checkpoint reuse, broader fuzzing and provider-backed comparison remain open. F6 still needs scanner triage, broader evaluation and release/rollback gates. Jules drafts not merged/validated are not evidence.
...
- **F3 Token Firewall — partial:** existing operation traces, bounded `compactResult`, `waitForTerminal` watcher, and compiler fragment hashes/deltas provide part of the foundation. PR #19 adds secret-pattern redaction before task result persistence and an on-demand paginated `mcg evidence <task-id> --type result` drill-through; PR #22 adds redaction before replay/evaluation stdout and stderr are persisted, while preserving numeric token measurements. PR #23 adds a normalized process-operation envelope with terminal status, timestamps, duration, exact output byte counts, SHA-256 digests and source provenance, without retaining command arguments; replay/evaluation records persist this envelope. PR #25 adds a Local Runtime read-only batch API: up to 20 typed operations, bounded concurrency/output, stable result ordering, sanitized per-operation failures, strict field validation and a 10-second network timeout ceiling. PR #26 adds executor tests for symlink escape, removed files and oversized inputs. PR #28 adds bounded SHA-256 `read-if-changed` evidence at the Local Runtime executor boundary (same hash returns the digest without file content; changed hash returns current sanitized content), with a 256 KB hard read cap. PR #29 exposes these operations through one stdio MCP tool and tests discovery/calls with the official client. The standalone product now has a project-scoped Codex MCP entry in `.codex/config.toml`, allow-listing only `mcg_read_batch` and requiring per-tool approval; a launcher integration test exercises `tools/list` and an actual bounded read through the official MCP client. `codex mcp get/list` recognizes the registration. This proves the local MCP protocol path, not a successful Codex model/tool turn: host approval policy blocked the prior non-interactive attempt, so provider live-session verification remains open. No model-roundtrip/token savings are claimed. Still open: live Codex tool-loop verification; broader hostile-input/fuzz coverage; Git checkpoint adoption by the router; and provider-backed comparative benchmark. No global Codex config was changed. Never change global Codex config without separate authorization.
- **F4 Clear reports for all 14 dashboard areas — accepted:** one report renderer covers Overview, History, Traces, Tasks, Agents, Tools, Plugins/Skills, MCPs, Context/Graph, Executions, Cache, Memory, Validation MCG and Alerts. Each report presents source, query time, measurement quality, scope/limitations, unavailable fields, escaped values and empty states. Automated and isolated browser checks verify report loading, selected navigation, deep-link reload, current/partial/unavailable status and no browser errors. PRs #8–9 add keyboard semantics, live loading/result/error announcements, stale-response protection and fresh reads. PR #12 scopes Cache/History and validation samples; PR #14 adds task-result evidence coverage; PR #16 distinguishes query time from source observation; PR #18 adds scope/limitations. F4 no longer includes visual or assistive-device acceptance; those belong to F5.
- **F5 Reference-faithful design, themes and accessibility — partial, not accepted:** exact light/dark semantic palettes and persistent Light/Dark/System selection are implemented. Isolated Playwright confirms the 1280×720 dashboard layout/reference section order, 320/375/768px without horizontal overflow, 44px minimum touch controls on mobile/tablet, theme switching, skip-link keyboard activation and no page errors. WCAG token checks verify text contrast ≥4.5:1, focus contrast ≥3:1 and reduced-motion handling. Visual review is structural, not pixel-diff. Remaining: screen-reader verification and physical touch-device QA. Do not control the user's active Chrome; use isolated browser/test tooling.
...
- **F6 Evaluation, benchmark and release gate — partial:** security-scanning baseline is merged and post-merge checks passed at `249cb391cd51c235f2489ffbb6f858c82774c336`; scanners remain report-only and ZAP passive-only. Read-only GitHub snapshot on 2026-09-25: 106 open code-scanning alerts (96 CodeQL, 10 Semgrep OSS), 0 open Dependabot security alerts; the 106 alerts remain untriaged. Main-commit Gitleaks artifact reports 0 findings; passive ZAP reported 11 instances (2 medium, 5 low, 4 informational). Paired validation-v2 ran 30/30 cases on real Codex CLI (`gpt-5.5`, medium), with exact provider measurements and real executor: baseline and MCG each achieved 100% task success, context recall and evidence grounding, with 0% hallucination. Total tokens: 1,028,032 baseline vs. 862,578 MCG (16.09% measured saving); context-token saving: 12.74%. Persisted suite `suite-1790326233861-72a1d05d` reports `VALIDATED`. This is evidence for this corpus only, not a universal guarantee or release approval. Still required: individual scanner triage and reviewed baselines, broader eval coverage and verified release/rollback checks. No unsupported savings claims or deployment.

### 49.2.1 Current implementation evidence (2026-09-25)

- F3 focused work is implemented in the isolated, unmerged branch `codex/mcg-f3-summary-redaction`: `compactResult` redacts all returned fields before truncation, enforces per-field caps, and preserves numeric token-economy metrics rather than masking them as credentials; unit/fuzz tests cover completed and blocked results. The Local Runtime adds a read-only `git-diff-if-changed` checkpoint for tracked staged+unstaged diffs; an unchanged SHA-256 returns only the digest, while a changed diff returns sanitized content. It disables external diff/textconv helpers and rejects truncated output so incomplete diffs cannot be checkpointed. Local validation (2026-09-25): MCG 52/52 tests; dashboard 18/18; Local Runtime 30/30 and strict typecheck; syntax/import 35 modules; sensitive scan 134 files; Jazzer 80,582 runs/11 seconds without a crash. `.codex/config.toml` now registers only the read-only MCP tool at project scope. The official MCP client test launches that wrapper and completes discovery plus a bounded read; `codex mcp get/list` sees the server, but a live Codex model/tool turn has not been proven. Not merged; F3 remains partial until that provider check and the other F3 gates pass.
- F6 benchmark details and remaining release gates are in the F6 status immediately above.
...
- Light mode rendered `rgb(242, 242, 247)`; dark mode rendered `rgb(0, 0, 0)`. At 320px, 375px and 768px, document width matched viewport; visible mobile/tablet controls were at least 44px.
- At 1280×720, isolated screenshot review confirmed reference section order and compact layout; validation metrics fit in the viewport. Overview shows one recent task with a “Ver todas” link; full task/alert data remains in reports.
- `/?view=History` reloads with History selected and report loaded. No page errors.
- Screen-reader output and physical touch-device interaction remain unverified; visual comparison was structural, not pixel-diff.
