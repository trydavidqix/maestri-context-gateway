...
The current MCG tracker reports phases 0–2 accepted, phase 3 partial, phase 4 partially implemented but not accepted, phase 5 implemented but not accepted, and phase 6 partial: **3/7 accepted = 43% for the MCG workstream only**. This is not the overall Maestri/Brain/Governance completion percentage. Dashboard has readable reports for all 14 registered views and a working light/dark/system selector. Isolated Playwright verified both palette backgrounds/meta colors, all 14 report views and 320px mobile without horizontal overflow. PRs #8–9 implement a single report renderer, report-heading focus, keyboard-scrollable task table with caption/scoped headers, polite loading/success/unavailable/error announcements, stale-response protection, and no-store refresh on each report visit. PR #12 adds explicit ALL_TIME/5,000-record scope for Cache/History and benchmark sample coverage against the 30-pair minimum, remaining unavailable when no valid pairs exist. PR #14 adds all-time task scope and exact result-file evidence coverage. PR #16 distinguishes report-generation time from actual last source observation for Cache/History. PR #18 adds explicit data scope and limitations to every view. Interactive browser checks now verify skip-link keyboard activation, all 14 views completing with selected state and loaded reports, the exact light theme background, 44px mobile controls, and no browser errors. Still open: screen-reader testing, touch-device behavior, and image-based reference-fidelity review. Jules drafts not merged/validated are not implementation evidence.
...
- **F3 Token Firewall — partial:** existing operation traces, bounded `compactResult`, `waitForTerminal` watcher, and compiler fragment hashes/deltas provide part of the foundation. PR #19 adds secret-pattern redaction before task result persistence and an on-demand paginated `mcg evidence <task-id> --type result` drill-through; PR #22 adds redaction before replay/evaluation stdout and stderr are persisted, while preserving numeric token measurements. PR #23 adds a normalized process-operation envelope with terminal status, timestamps, duration, exact output byte counts, SHA-256 digests and source provenance, without retaining command arguments; replay/evaluation records persist this envelope. PR #25 adds a Local Runtime read-only batch API: up to 20 typed operations, bounded concurrency/output, stable result ordering, sanitized per-operation failures, strict field validation and a 10-second network timeout ceiling. PR #26 adds executor tests for symlink escape, removed files and oversized inputs. PR #28 adds bounded SHA-256 `read-if-changed` evidence at the Local Runtime executor boundary (same hash returns the digest without file content; changed hash returns current sanitized content), with a 256 KB hard read cap. PR #29 exposes these operations through one stdio MCP tool and tests discovery/calls with the official client. PR #37 adds Jazzer.js fuzzing for credential redaction in GitHub Actions; this is targeted redaction coverage, not broad hostile-input fuzzing or provider/tool-loop integration. **Focused follow-up implemented locally, not merged:** exported `compactResult` now redacts every summary value before serialization/truncation and enforces field limits; tests cover result, validation, commit, evidence, task id, blocker and owner-needed fields, and the Jazzer target covers completed and blocked summaries. Local validation passes: 51/51 tests, 34 syntax/import modules, sensitive scan (133 files), and 44,797 Jazzer executions in the final local run. This does not close F3. No provider session was registered or run, and no model-roundtrip or token savings are claimed. Still open: merge and CI verification of the focused change; optional project-scoped provider registration/live-session verification; broader hostile-input/fuzz coverage; and Git checkpoint reuse. The paired provider-backed benchmark is now recorded under F6. No global Codex config was changed. Never change global Codex config without separate authorization.
- **F4 Clear reports for all 14 dashboard areas — partial, not accepted:** shared report shell and readable recursive fields/lists cover Overview, History, Traces, Tasks, Agents, Tools, Plugins/Skills, MCPs, Context/Graph, Executions, Cache, Memory, Validation MCG and Alerts. Reports show source, query time, measurement quality, explicit per-view data scope/limitations, unavailable fields, safe escaped content and empty states. Isolated Playwright verified all 14 views complete, selected state and report content without browser errors. PRs #8–9 added accessible report navigation, live loading/result/error announcements, stale-response protection, and fresh no-store data on each visit. PR #12 adds declared ALL_TIME/5,000-record scope on Cache/History and measured coverage of observed validation pairs against the explicit 30-pair minimum. PR #14 adds task-result evidence coverage using the observed task count as denominator. PR #16 labels report query time separately from actual latest Cache/History observation. Remaining gates: screen-reader testing and complete image-based visual QA.
...
- **F6 Evaluation, benchmark and release gate — partial:** the security-scanning baseline is implemented in PR #31 (CodeQL, Semgrep, Gitleaks, OSV-Scanner and Dependabot), PR #36 (isolated ZAP passive baseline) and PR #37 (Jazzer.js redaction fuzzing); all are merged to `main`, and their post-merge workflows passed at `249cb391cd51c235f2489ffbb6f858c82774c336`. These scanners currently report findings without blocking CI; ZAP is passive-only. On 2026-09-25, paired validation-v2 ran 30/30 cases using real Codex CLI (`gpt-5.5`, medium), with exact provider measurements and a real executor: baseline and MCG each achieved 100% task success, context recall and evidence grounding, with 0% hallucination. Total measured tokens were 1,028,032 baseline vs. 862,578 MCG (16.09% measured saving); context-token saving was 12.74%. Persisted suite `suite-1790326233861-72a1d05d` reports `VALIDATED`. This supports the paired benchmark and measured non-regression for this corpus only, not a universal savings guarantee or release approval. Still required: triage existing scanner findings and establish baselines before promoting selected checks to gates; screen-reader/touch and image-based reference QA; release/rollback checks; and broader evaluation beyond this 30-case dataset. No unsupported savings claims or deployment.

### 49.2.1 Current implementation evidence (2026-09-25)

- F3 focused work is implemented in the isolated, unmerged branch `codex/mcg-f3-summary-redaction`: `compactResult` redacts all returned fields before truncation, enforces per-field caps, and preserves numeric token-economy metrics rather than masking them as credentials; unit/fuzz tests cover completed and blocked results. The Local Runtime adds a read-only `git-diff-if-changed` checkpoint for tracked staged+unstaged diffs; an unchanged SHA-256 returns only the digest, while a changed diff returns sanitized content. It disables external diff/textconv helpers and rejects truncated output so incomplete diffs cannot be checkpointed. Local validation: MCG 51/51 tests; Local Runtime 29/29 tests and strict typecheck; syntax/import 34 modules; sensitive scan 133 files; Jazzer 44,797 executions. Not merged; F3 remains partial until integration/CI and other F3 gates pass.
