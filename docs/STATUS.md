...
- Phases 0–2 are accepted. Phase 3 Token Firewall is partial and not accepted; PR #19 adds secret-redacted task-result drill-through, PR #22 extends redaction to replay/evaluation artifacts, PR #23 adds normalized process status/output provenance, and PR #25 adds a bounded typed Local Runtime batch API. The batch API is not yet wired into a provider/tool loop, so reduced model round-trips have not been proven; end-to-end reuse and provider benchmark gates remain open. Phases 4–6 (14 reports, reference-faithful dual theme/accessibility, final eval/benchmark/release) remain open.
