...
- This work is an additive part of MCG release/evaluation readiness, not a replacement for the dashboard/Token Firewall plan and not a separate completion denominator. MCG remains **3/7 accepted phases = 43%**; F3 is partial, F4 is partial/not accepted, F5 is implemented/not accepted, and F6 is partial.
- Security tooling merged to this repository's `main`: PR #31 adds CodeQL, Semgrep, Gitleaks, OSV-Scanner and Dependabot; PR #36 adds an isolated ZAP passive baseline against the loopback dashboard; PR #37 adds Jazzer.js credential-redaction fuzzing. All post-merge MCG, CodeQL, Semgrep, Gitleaks, OSV-Scanner, ZAP, Jazzer.js and publish checks passed for `249cb391cd51c235f2489ffbb6f858c82774c336`; that is the verified `main`/`origin/main` source commit. The main checkout currently also has uncommitted edits to this plan and status document.
- Scanner workflows are currently report-only; ZAP is passive-only. Their green workflow status proves execution and report upload, not absence or remediation of findings. GitHub's current code-scanning snapshot has 96 CodeQL alerts (3 critical, 93 high) and 10 Semgrep alerts. Dependabot alert count could not be verified with the current GitHub CLI permissions; 4 Dependabot update PRs are open. Triage/baseline and promotion of selected checks to blocking gates remain pending. ZAP's passive baseline reports warning groups; see the retained Actions artifact before deciding UI/security changes.
- PR #37's Jazzer.js target covers credential redaction only. The isolated, unmerged branch `codex/mcg-f3-summary-redaction` now also hardens exported `compactResult` and implements tracked staged/unstaged Git diff checkpoints; local runtime tests pass 29/29 with strict typecheck, the MCG suite passes 51/51, syntax/import parses 34 modules, the sensitive scan passes (133 files), and Jazzer completed 44,797 executions. This does not prove live provider/tool-loop integration or close broader hostile-input fuzz coverage. Isolated Chromium now verifies all 14 dashboard reports, keyboard skip-link, selected navigation state, theme selection and 320px no-overflow layout; screen-reader, touch and reference-image checks remain open. Provider registration/live-session verification and merge/CI for the local checkpoint remain open; F6 triage/release checks remain open as stated in §49 of `MASTER_BLUEPRINT_CANONICAL.md`.
- Paired provider-backed validation-v2 is complete for 30/30 cases; see persisted suite `suite-1790326233861-72a1d05d` and the detailed measurements in §49.2. This does not change the accepted-phase count: MCG remains 3/7 (43%).
