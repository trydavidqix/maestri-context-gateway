...
This is an isolated migration branch; GitHub integration is still pending. Local architecture and test gates have been rerun after the moves. Live Codex eval commands are intentionally excluded because they execute real provider requests; runner behavior is covered by unit/integration tests.
...
- **Remote:** Migration PR and CI are pending. Repository is public, `main` is default, and no branch protection/ruleset was observed; preserve those settings unless a separate approved change is made.
