...
## Current MCG implementation status — 2026-09-25

- Canonical MCG plan: `MASTER_BLUEPRINT_CANONICAL.md` §49. F0–F2 and F4 accepted: **4/7 = 57%**. F3, F5 and F6 remain partial; this percentage applies only to MCG.
- Isolated branch `codex/mcg-f3-summary-redaction` locally hardens `compactResult` with redaction and field caps, and adds a read-only tracked staged/unstaged Git diff SHA checkpoint exposed via MCP. It returns only a digest for unchanged output, sanitizes changed output, disables external diff/textconv helpers, and rejects truncated output. This work is not merged.
- Local validation (2026-09-25): MCG 52/52 tests, dashboard 18/18, Local Runtime 30/30 and strict typecheck; syntax/import 35 modules; sensitive scan 134 files; Jazzer 80,582 runs/11 seconds without a crash. The added MCP launcher integration test starts the project-scoped Codex launcher and performs `tools/list` plus a bounded `mcg_read_batch` call using the official MCP TypeScript client; it also guards against Node's `DEP0190` shell-argument warning.
- Codex integration remains partial: `.codex/config.toml` is project-scoped, allow-lists only `mcg_read_batch`, and sets per-tool `approval_mode = "approve"`. `codex mcp get/list` recognizes the configured server. No live Codex model/tool turn is claimed: a previous non-interactive call was blocked by the host's approval policy, and the follow-up attempt did not complete a tool call. The project config has not been copied into global Codex settings.
- Persisted paired validation suite `suite-1790326233861-72a1d05d`: 30/30 cases on real Codex CLI (`gpt-5.5`, medium), exact token measurement and real executor. Baseline/MCG each achieved 100% success, recall and grounding, 0% hallucination. Total tokens 1,028,032 vs. 862,578 (16.09% saving); context-token saving 12.74%. Evidence is limited to this evaluation corpus.
- F4 (all 14 reports) is accepted after isolated Chromium verified report loading, selected navigation and deep-link reload. Screenshot review at 1280×720 matches reference section order and compact layout; report data remains complete behind “Ver todas” links.
- Browser also verified light/dark backgrounds, 320/375/768px without horizontal overflow, minimum 44px mobile/tablet controls, keyboard skip-link and no page errors. Screen-reader and physical touch-device checks remain open under F5.
- Remaining: merge/CI for local F3 work; provider live-session integration; broader hostile-input fuzz and Git checkpoint adoption; F5 screen-reader/touch QA; F6 scanner triage, broader evaluation and release/rollback gates.

## Scanner snapshot — 2026-09-25

- Read-only GitHub code-scanning API snapshot for the repository default-branch commit `249cb391cd51c235f2489ffbb6f858c82774c336`: 106 open alerts (96 CodeQL, 10 Semgrep OSS). The alert set is not yet individually triaged; scanner severity is not a verdict. Dependabot security alerts: 0 open.
- Rule inventory: 89 `js/path-injection`; 3 mutable GitHub Actions tags; 2 command-line injection; 2 disabled TLS verification; 2 TLS bypass; and 8 single findings across non-literal regexp, unsafe format string, bad tag filter, request forgery, SQL injection, and three pnpm policy rules. This is an inventory, not a dismissal/baseline.
- Main-commit security workflow `36053933334` and CodeQL workflow `36053931376` completed successfully on `249cb391...`; the security workflow is report-only (`continue-on-error`) and ZAP is passive baseline. Gitleaks SARIF artifact was parsed without exposing secret content and contained 0 findings. ZAP artifact contained 11 instances: 2 medium-confidence header findings, 5 low-confidence header findings, and 4 informational instances. These results do not approve release or establish that the 106 code-scanning alerts are false positives.
- Next F6 work: triage alert evidence against exact source paths, define a reviewed baseline without hiding live findings, expand evaluation coverage, and specify/verify release plus rollback gates. No security alert was dismissed or modified.
