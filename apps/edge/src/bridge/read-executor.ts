...
import type { RuntimeCorrelation } from "@nexus-brain/contracts/runtime";
...
export class ReadOnlyLocalExecutor {
  private readonly gitAdapter: GitReadAdapter;
  private readonly maxFileBytes: number;
  private readonly maxSearchResults: number;

  constructor(private readonly options: ReadExecutorOptions) {
    this.gitAdapter = new GitReadAdapter({ workspaceRoot: options.workspaceRoot, commandRunner: options.commandRunner, executable: options.gitExecutable });
  ...
  async gitStatus(correlation: RuntimeCorrelation): Promise<ExecutorResult<string>> {
    return this.result("git.status", await this.gitAdapter.status(correlation));
  }

  async gitDiff(correlation: RuntimeCorrelation): Promise<ExecutorResult<string>> {
    return this.result("git.diff", await this.gitAdapter.diff(correlation));
  }
  ...
  private async gitDiffIfChanged(correlation: RuntimeCorrelation, expectedSha256?: string): Promise<ExecutorResult<{ sha256: string; unchanged: boolean; content?: string }>> {
    const diff = await this.gitAdapter.diffFromHead(correlation);
  ...
  async gitLog(correlation: RuntimeCorrelation, count = 20): Promise<ExecutorResult<string>> {
    return this.result("git.log", await this.gitAdapter.log(correlation, count));
  }
