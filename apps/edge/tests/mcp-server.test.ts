...
const serverPath = join(packageRoot, "src", "bridge", "mcp-server.ts");
const workspaceRoot = fileURLToPath(new URL("../../../", import.meta.url));
const codexLauncherPath = join(workspaceRoot, "tooling", "scripts", "run-edge-mcp.mjs");
...
describe("Local Runtime MCP read-only batch tool", () => {
  it("starts the project-scoped Codex launcher and serves the read-only MCP tool", async () => {
    const env = Object.fromEntries(Object.entries({
      ...
      TEMP: process.env.TEMP,
      TMP: process.env.TMP,
      ComSpec: "C:\\invalid\\untrusted-shell.cmd",
    }).filter((entry): entry is [string, string] => typeof entry[1] === "string"));
    const transport = new StdioClientTransport({
      command: process.execPath,
    ...
    try {
      ...
      const result = await client.callTool({
        name: "mcg_read_batch",
        arguments: { operations: [{ id: "manifest", kind: "read", path: "apps/edge/package.json" }] },
      });
      ...
      expect(payload.results[0]?.value?.value).toContain("@nexus-brain/edge");
    } finally {
      await client.close();
    }
