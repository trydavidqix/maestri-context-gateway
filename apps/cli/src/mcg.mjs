...
import { dispatch, ensureLayout, ingest, loadState, listTasks, compactResult, removeTask, sliceEvidence, waitForTerminal, stats, ROOT, saveState } from '@nexus-brain/context-gateway/core';
import { acceptWireMutation, loadWireConfig, openWireFeed, summarizeWireSnapshot, wireRequest, wireSnapshot } from '@nexus-brain/edge/wire';
import { createDashboardServer } from '@nexus-brain/control-center';
import { replayTask, compareReplay } from '@nexus-brain/execution/replay';
import { discoverProviderConfigs } from '@nexus-brain/context-gateway/provider-discovery';
import { refreshRegistries } from '@nexus-brain/context-gateway/registry';
import { startDaemonControl, stopDaemon } from '@nexus-brain/edge/daemon-control';
...
try {
  ...
  if (command === 'doctor') {
    ...
    const state = await loadState(args[0]); state.internal_state = 'CANCELLED'; state.updated_at = new Date().toISOString(); await saveState(state); print({ task_id: state.task_id, state: state.internal_state });
